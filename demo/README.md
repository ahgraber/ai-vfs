...
| `AIVFS_TOOLS`               | `all`                       | tool surfaces (comma-ok): `codemode` (`run_code`), `files` (native file tools), `bash` (`run_bash`), `python` (deprecated `run_python`); `all` = `codemode,bash`. `files`+`codemode` exposes the file tools only via `run_code` (logs a warning) |
