...
- `cat(path)` → `session.read(path)` decoded as strict UTF-8; undecodable content yields a structured error and no anchors.
  ...
  Leading `//` in a path is POSIX-canonical (normalised to `/` by `posixpath.normpath`) and is permission-checked like any other path.
...
2. **Write:** Construct replacement content; `edit()` SHALL always pass the anchor-validated `version_number` as `expected_version` to `session.write`.
   ...
   On success, the path's anchor state is atomically REPLACED with the longest-common-block (difflib) reconciliation result (no prior `invalidate` call).
